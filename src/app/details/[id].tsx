...
import { getById, neighbors } from '@/data/catalog';
  ...
  return (
    <Screen tabInset={false} scroll padded={false}>
      ...
      <View style={[styles.stage, { backgroundColor: colors.panelHot }]}>
        <Image source={{ uri: item.url }} style={StyleSheet.absoluteFill} contentFit="cover" transition={350} />
      ...
      <View style={{ paddingHorizontal: 20, paddingTop: 16 }}>
        <View style={styles.tools}>
          ...
          <IconButton label="Share" onPress={() => setNotice('Share preview — ready in this UI only.')}>
            <Mark name="share" size={16} />
          </IconButton>
          <IconButton
            label="Save"
            onPress={() => setNotice(item.mediaType === 'image' ? 'Image marked as saved.' : 'Video marked as saved.')}
          >
